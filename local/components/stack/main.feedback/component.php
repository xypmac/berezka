...
$arParams["EVENT_TITLE"] = trim($arParams["EVENT_TITLE"]);


if(strlen($arParams["EVENT_TITLE"]) <= 0)
	$arParams["EVENT_TITLE"] = "FEEDBACK_FORM";
...
if($_SERVER["REQUEST_METHOD"] == "POST" && $_POST["submit"] <> '' && (!isset($_POST["PARAMS_HASH"]) || $arResult["PARAMS_HASH"] === $_POST["PARAMS_HASH"]))
{
	...
	if(check_bitrix_sessid())
	{
		if(empty($arParams["REQUIRED_FIELDS"]) || !in_array("NONE", $arParams["REQUIRED_FIELDS"]))
		{
			...
			if((empty($arParams["REQUIRED_FIELDS"]) || in_array("PHONE", $arParams["REQUIRED_FIELDS"])) && mb_strlen($_POST["phone"]) <= 3)
		...
		if(empty($arResult["ERROR_MESSAGE"]))
		{
			...
			$_SESSION["MF_TITLE"] = htmlspecialcharsEx($_POST["title"]);
		...
		$arResult["TITLE"] = htmlspecialcharsEx($_POST["title"]);
...
if(empty($arResult["ERROR_MESSAGE"]))
{
	if($USER->IsAuthorized())
	{
		...
		$arResult["TITLE"] = htmlspecialcharsEx($USER->GetEmail());
		$arResult["PHONE"] = htmlspecialcharsEx($USER->GetEmail());
	}
	else
	{
		if($_SESSION["MF_NAME"] <> '')
		...
		if(strlen($_SESSION["MF_TITLE"]) <> '')
			$arResult["TITLE"] = htmlspecialcharsEx($_SESSION["MF_TITLE"]);
		if(strlen($_SESSION["MF_PHONE"]) <> '')
			$arResult["PHONE"] = htmlspecialcharsEx($_SESSION["MF_PHONE"]);
	}
}
